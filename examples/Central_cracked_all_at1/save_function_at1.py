...
def save_central_crack(S, h, Data, a0=0.5, case='AT1'):
    ...
    gc_factor = np.full_like(displacement_quarter, 1 + 2*h/(c0*Data.l))
    ...
    # gamma_phi = S.energy_files['total.energy']["gamma_phi"]
    # gamma_gradphi = S.energy_files['total.energy']["gamma_gradphi"]

    # gradient_factor = np.where(
    #     gamma_phi < gamma_gradphi,
    #     1.0,
    #     0.5 * (1.0 + np.abs(gamma_phi) / gamma_gradphi)
    # )

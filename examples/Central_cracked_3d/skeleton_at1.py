r"""
.. _ref_skeleton_3d_at1:

Skeletonization — crack area measurement from 3D AT1 simulation
---------------------------------------------------------------

AT1 counterpart of :ref:`ref_skeleton_3d`. Measures the crack area from the phase-field
VTU output of :ref:`ref_central_cracked_3d_simulation` using the same 2D image-processing
pipeline. The result is used as the **Skeleton** reference correction method for the AT1
benchmark.

**Workflow** (identical to :ref:`ref_skeleton_3d`)

1. Render each VTU file to a PNG image, marking :math:`\phi > 0.95` in red.
2. Auto-detect the specimen bounding box and compute pixel-to-mm scale factors.
3. Skeletonize the red crack pixels and extract physical :math:`(x, y)` coordinates.
4. Fit a smoothing spline and compute its arc length → crack length in mm.
5. Multiply by :math:`t = 0.05` mm → crack area in mm².
6. Interpolate to integer step indices and save.

**Output files** (written to ``results_simulation_3d_AT1/crack_measurement/``)
...
- ``interpolated_step_time_crack_length.txt`` — crack length interpolated to integer
  step indices; read by :ref:`ref_central_cracked_3d_simulation` to compute
...
# folder_simulation = "results_simulation_3d_AT1"
# input_folder = os.path.join(folder_simulation, "paraview-solutions_vtu")
# output_folder = os.path.join(folder_simulation, "crack_measurement")

r"""
.. _ref_central_cracked_3d_simulation:

3D central cracked specimen — AT1 regularization
-------------------------------------------------

Three-dimensional phase-field fracture simulation of the central cracked tension specimen
with **AT1** regularization (:math:`\alpha(\phi) = \phi`, :math:`c_0 = 8/3`), companion
to :ref:`ref_central_cracked_3d_simulation_at2`.
...
- Penalization: :math:`\rho = 10^8` (enforces :math:`\phi \geq 0` for AT1)
...
- ``results_corrected_bourdin.pff`` — Bourdin factor :math:`1 + h/(c_0 l)` with :math:`c_0 = 8/3`.
- ``results_corrected_gradient.pff`` — DGCM.
- ``results_corrected_skeleton.pff`` — skeletonization reference (requires
  ``crack_measurement/interpolated_step_time_crack_length.txt`` from :ref:`ref_skeleton_3d_at1`).
...
Data = Input(E=210.0,                       
             ...
             results_folder_name="results_simulation_3d_AT1")
...
# Physical Surface("top_left", 47) = {31};
# Physical Surface("bottom_left", 48) = {43};

# bottom = facet_markers.find(45)
# top = facet_markers.find(46)
# left_top = facet_markers.find(47)
# left_bottom = facet_markers.find(48)
...
def save_central_crack(S, h, Data, a0=0.5):
    ...
    # ----------------------------------
    c0=8.0/3.0
    gc_factor = np.full_like(displacement_quarter, 1 + h/(c0*Data.l))
    ...
    gradient_factor = 0.5 * (1.0 + S.energy_files['total.energy']["gamma_phi"]/S.energy_files['total.energy']["gamma_gradphi"])
...
def plot_comparison_results(data_obj):
    ...
    step_initial = 1168 - 1       #20
    step_mid = 2320 - 1          #40
    step_final = 2600 - 1        #60
    ...
    skel_mid = 2.98789425e-01 * thickness  - a0_val
    skel_final = 3.29689876e-01 * thickness  - a0_val
